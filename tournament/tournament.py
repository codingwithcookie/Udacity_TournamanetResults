...
def playerStandings():
    ...
    db, cur = connect()
    # Database execution
    SQL = """SELECT PLAYER.ID, PLAYER.NAME, COALESCE(SUM(CASE WHEN
    PLAYER.ID = MATCH.WINNER THEN 1 ELSE 0 END), 0) AS WINS, COUNT(MATCH.ID)
    FROM PLAYER LEFT OUTER JOIN MATCH ON
    (PLAYER.ID = MATCH.WINNER OR PLAYER.ID = MATCH.LOSER)
...
def swissPairings():
    ...
    swisslist = []
    temptuple = ()
    # Set count to zero and get player standings
    count = 0
